    ...
    public void CalculateScores()
    {
        foreach (GameObject bottle in bottles)
        {
            if (bottle != null)
            {
                ...
                // 범위에 따른 점수 부여
                if (distance <= 3f)  // 3cm 이내
                {
                    scoreToAdd = 20;  // 최고 점수
                }
                else if (distance <= 5f)  // 5cm 이내
                {
                    scoreToAdd = 15;  // 중간 점수
                }
                else if (distance <= 8f)  // 8cm 이내
                {
                    scoreToAdd = 10;  // 낮은 점수
                }
                ...
                if (scoreToAdd > 0)
                {
                    if (bottle.GetComponent<Bottle>().team == 0)  // Red 팀
                    {
                        GameManager.Instance.AddScoreToRedPlayer(scoreToAdd);
                    }
                    else if (bottle.GetComponent<Bottle>().team == 1)  // Blue 팀
                    {
